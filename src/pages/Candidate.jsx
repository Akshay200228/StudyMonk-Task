...
// Extract the unique locations from candidatesData
...
const Candidate = () => {
  ...
  return (
    <div className={`${styles.paddingY} mt-14 flex flex-col lg:flex-row h-screen`}>
      ...
      <div className="w-full px-4 mt-4 overflow-y-auto">
        <h2 className="text-2xl font-bold text-white">Candidate List</h2>
        {filteredCandidates.length > 0 ? (
          <ul className="mt-4 text-white">
            {filteredCandidates.map((candidate) => (
              <li key={candidate.id} className="mb-8">
                <div className="flex items-center">
                  ...
                  />
                  <div>
                    <h3 className="text-lg font-semibold">{candidate.location}</h3>
